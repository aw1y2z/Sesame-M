...
public class PlantSceneIdMap {
    ...
    public synchronized static void load() {
        idMap.clear();
        try {
            String body = FileUtil.readFromFile(FileUtil.getPlantSceneIdMapFile());
            if (!body.isEmpty()) {
    ...
    public synchronized static boolean save() {
        return FileUtil.write2File(JsonUtil.toJsonString(idMap), FileUtil.getPlantSceneIdMapFile());
    }
