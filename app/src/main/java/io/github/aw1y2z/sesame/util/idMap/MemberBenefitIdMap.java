...
public class MemberBenefitIdMap {
    ...
    public static synchronized void load(String userId) {
        idMap.clear();
        try {
            String body = FileUtil.readFromFile(FileUtil.getMemberBenefitIdMapFile(userId));
            if (!body.isEmpty()) {
    ...
    public static synchronized boolean save(String userId) {
        return FileUtil.write2File(JsonUtil.toJsonString(idMap), FileUtil.getMemberBenefitIdMapFile(userId));
    }
}
