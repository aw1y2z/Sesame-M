...
public class NewAncientTreeIdMap {
    ...
    public synchronized static void load() {
        idMap.clear();
        try {
            String body = FileUtil.readFromFile(FileUtil.getNewAncientTreeIdMapFile());
            if (!body.isEmpty()) {
    ...
    public synchronized static boolean save() {
        return FileUtil.write2File(JsonUtil.toJsonString(idMap), FileUtil.getNewAncientTreeIdMapFile());
    }
