...
public class AntForestHuntTaskListMap {
    ...
    public synchronized static void load() {
        idMap.clear();
        try {
            String body = FileUtil.readFromFile(FileUtil.getAntForestHuntTaskListMapFile());
            if (!body.isEmpty()) {
    ...
    public synchronized static boolean save() {
        return FileUtil.write2File(JsonUtil.toJsonString(idMap), FileUtil.getAntForestHuntTaskListMapFile());
    }
