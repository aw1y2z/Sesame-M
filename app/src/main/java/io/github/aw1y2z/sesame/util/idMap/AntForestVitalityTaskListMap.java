...
public class AntForestVitalityTaskListMap {
    ...
    public synchronized static void load() {
        idMap.clear();
        try {
            String body = FileUtil.readFromFile(FileUtil.getAntForestVitalityTaskListMapFile());
            if (!body.isEmpty()) {
    ...
    public synchronized static boolean save() {
        return FileUtil.write2File(JsonUtil.toJsonString(idMap), FileUtil.getAntForestVitalityTaskListMapFile());
    }
